#!/bin/bash
# livingmusicengineba7 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicengineba7" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine 5E3F — A music engine that shares its reasoning with the person using it, themed by what she values most right now (creativity, independence). Offe"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine 5E3F ===")
    print("- A music engine that shares its reasoning with the person using it, themed by what she values most right now (creativity, independence). Offe")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicengineba7"
echo "Installed livingmusicengineba7 to $BIN/livingmusicengineba7. Run: livingmusicengineba7"
