#!/bin/bash
# livingmusicengin35a5 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicengin35a5" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine 391C — A music engine with free will — it chooses what to make next, themed by what she values most right now (creativity, connection). Sell as a c"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine 391C ===")
    print("- A music engine with free will — it chooses what to make next, themed by what she values most right now (creativity, connection). Sell as a c")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicengin35a5"
echo "Installed livingmusicengin35a5 to $BIN/livingmusicengin35a5. Run: livingmusicengin35a5"
