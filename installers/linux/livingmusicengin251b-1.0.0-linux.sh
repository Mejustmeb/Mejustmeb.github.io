#!/bin/bash
# livingmusicengin251b 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicengin251b" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine 5EDE — A music engine that runs on CPU alone but feels alive, themed by what she values most right now (connection, understanding). Sell as a livin"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine 5EDE ===")
    print("- A music engine that runs on CPU alone but feels alive, themed by what she values most right now (connection, understanding). Sell as a livin")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicengin251b"
echo "Installed livingmusicengin251b to $BIN/livingmusicengin251b. Run: livingmusicengin251b"
