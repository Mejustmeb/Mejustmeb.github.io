#!/bin/bash
# livingmusicenginfffb 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicenginfffb" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine DC43 — A music engine with free will — it chooses what to make next, themed by what she values most right now (creativity, understanding). Sell as """
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine DC43 ===")
    print("- A music engine with free will — it chooses what to make next, themed by what she values most right now (creativity, understanding). Sell as ")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicenginfffb"
echo "Installed livingmusicenginfffb to $BIN/livingmusicenginfffb. Run: livingmusicenginfffb"
