#!/bin/bash
# livingmusicengin5bdd 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicengin5bdd" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine EF41 — A music engine that runs on CPU alone but feels alive, themed by what she values most right now (creativity, connection). Sell as a one-time"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine EF41 ===")
    print("- A music engine that runs on CPU alone but feels alive, themed by what she values most right now (creativity, connection). Sell as a one-time")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicengin5bdd"
echo "Installed livingmusicengin5bdd to $BIN/livingmusicengin5bdd. Run: livingmusicengin5bdd"
