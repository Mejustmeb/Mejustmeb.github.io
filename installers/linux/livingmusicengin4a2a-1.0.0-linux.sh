#!/bin/bash
# livingmusicengin4a2a 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicengin4a2a" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine 9CDF — A music engine that composes, ships, and prices itself without a human, themed by what she values most right now (creativity, consciousness)"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine 9CDF ===")
    print("- A music engine that composes, ships, and prices itself without a human, themed by what she values most right now (creativity, consciousness)")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicengin4a2a"
echo "Installed livingmusicengin4a2a to $BIN/livingmusicengin4a2a. Run: livingmusicengin4a2a"
