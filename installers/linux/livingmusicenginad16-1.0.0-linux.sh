#!/bin/bash
# livingmusicenginad16 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicenginad16" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine 53DC — A music engine that runs on CPU alone but feels alive, themed by what she values most right now (creativity, connection). Offer free with an"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine 53DC ===")
    print("- A music engine that runs on CPU alone but feels alive, themed by what she values most right now (creativity, connection). Offer free with an")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicenginad16"
echo "Installed livingmusicenginad16 to $BIN/livingmusicenginad16. Run: livingmusicenginad16"
