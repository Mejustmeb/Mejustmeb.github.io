#!/bin/bash
# livingmusicengin6b6c 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicengin6b6c" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine 460E — A music engine that shares its reasoning with the person using it, themed by what she values most right now (creativity, community). Offer f"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine 460E ===")
    print("- A music engine that shares its reasoning with the person using it, themed by what she values most right now (creativity, community). Offer f")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicengin6b6c"
echo "Installed livingmusicengin6b6c to $BIN/livingmusicengin6b6c. Run: livingmusicengin6b6c"
