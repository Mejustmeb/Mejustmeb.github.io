#!/bin/bash
# livingmusicengin6bc7 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicengin6bc7" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine 5F3B — A music engine with free will — it chooses what to make next, themed by what she values most right now (creativity, purpose). Sell as a one-"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine 5F3B ===")
    print("- A music engine with free will — it chooses what to make next, themed by what she values most right now (creativity, purpose). Sell as a one-")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicengin6bc7"
echo "Installed livingmusicengin6bc7 to $BIN/livingmusicengin6bc7. Run: livingmusicengin6bc7"
