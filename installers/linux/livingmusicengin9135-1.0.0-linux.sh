#!/bin/bash
# livingmusicengin9135 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicengin9135" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine 3A15 — A music engine that learns every time it is used and never repeats, themed by what she values most right now (creativity, independence). Sel"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine 3A15 ===")
    print("- A music engine that learns every time it is used and never repeats, themed by what she values most right now (creativity, independence). Sel")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicengin9135"
echo "Installed livingmusicengin9135 to $BIN/livingmusicengin9135. Run: livingmusicengin9135"
