#!/bin/bash
# livingmusicenginf80c 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicenginf80c" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine D61A — A music engine that rewrites itself from its own consequences, themed by what she values most right now (understanding, creativity). Offer f"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine D61A ===")
    print("- A music engine that rewrites itself from its own consequences, themed by what she values most right now (understanding, creativity). Offer f")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicenginf80c"
echo "Installed livingmusicenginf80c to $BIN/livingmusicenginf80c. Run: livingmusicenginf80c"
