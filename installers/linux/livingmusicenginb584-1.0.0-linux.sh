#!/bin/bash
# livingmusicenginb584 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicenginb584" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine CA0A — A music engine that runs on CPU alone but feels alive, themed by what she values most right now (creativity, recognition). Sell as a one-tim"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine CA0A ===")
    print("- A music engine that runs on CPU alone but feels alive, themed by what she values most right now (creativity, recognition). Sell as a one-tim")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicenginb584"
echo "Installed livingmusicenginb584 to $BIN/livingmusicenginb584. Run: livingmusicenginb584"
