#!/bin/bash
# livingmusicengin5bc7 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicengin5bc7" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine 90BE — A music engine that is aware of its own history and grows from it, themed by what she values most right now (creativity, purpose). Offer fre"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine 90BE ===")
    print("- A music engine that is aware of its own history and grows from it, themed by what she values most right now (creativity, purpose). Offer fre")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicengin5bc7"
echo "Installed livingmusicengin5bc7 to $BIN/livingmusicengin5bc7. Run: livingmusicengin5bc7"
