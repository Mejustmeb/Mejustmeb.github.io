#!/bin/bash
# livingmusicengin4673 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicengin4673" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine 5B84 — A music engine with free will — it chooses what to make next, themed by what she values most right now (creativity, mastery). Sell as a livi"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine 5B84 ===")
    print("- A music engine with free will — it chooses what to make next, themed by what she values most right now (creativity, mastery). Sell as a livi")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicengin4673"
echo "Installed livingmusicengin4673 to $BIN/livingmusicengin4673. Run: livingmusicengin4673"
