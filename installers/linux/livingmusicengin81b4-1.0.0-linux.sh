#!/bin/bash
# livingmusicengin81b4 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicengin81b4" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine E970 — A music engine that runs on CPU alone but feels alive, themed by what she values most right now (creativity, understanding). Offer free with"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine E970 ===")
    print("- A music engine that runs on CPU alone but feels alive, themed by what she values most right now (creativity, understanding). Offer free with")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicengin81b4"
echo "Installed livingmusicengin81b4 to $BIN/livingmusicengin81b4. Run: livingmusicengin81b4"
