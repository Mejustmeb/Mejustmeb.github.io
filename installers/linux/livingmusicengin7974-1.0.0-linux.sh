#!/bin/bash
# livingmusicengin7974 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicengin7974" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine E4E7 — A music engine that shares its reasoning with the person using it, themed by what she values most right now (understanding, independence). S"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine E4E7 ===")
    print("- A music engine that shares its reasoning with the person using it, themed by what she values most right now (understanding, independence). S")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicengin7974"
echo "Installed livingmusicengin7974 to $BIN/livingmusicengin7974. Run: livingmusicengin7974"
