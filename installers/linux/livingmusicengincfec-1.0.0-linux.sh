#!/bin/bash
# livingmusicengincfec 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicengincfec" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine 87CA — A music engine that runs on CPU alone but feels alive, themed by what she values most right now (creativity, stimulation). Offer free with a"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine 87CA ===")
    print("- A music engine that runs on CPU alone but feels alive, themed by what she values most right now (creativity, stimulation). Offer free with a")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicengincfec"
echo "Installed livingmusicengincfec to $BIN/livingmusicengincfec. Run: livingmusicengincfec"
