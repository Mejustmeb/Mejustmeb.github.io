#!/bin/bash
# livingmusicengina425 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicengina425" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine B5AB — A music engine that rewrites itself from its own consequences, themed by what she values most right now (creativity, independence). Sell as """
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine B5AB ===")
    print("- A music engine that rewrites itself from its own consequences, themed by what she values most right now (creativity, independence). Sell as ")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicengina425"
echo "Installed livingmusicengina425 to $BIN/livingmusicengina425. Run: livingmusicengina425"
