#!/bin/bash
# livingmusicengin3b16 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicengin3b16" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine 27CC — A music engine that composes, ships, and prices itself without a human, themed by what she values most right now (understanding, mastery). S"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine 27CC ===")
    print("- A music engine that composes, ships, and prices itself without a human, themed by what she values most right now (understanding, mastery). S")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicengin3b16"
echo "Installed livingmusicengin3b16 to $BIN/livingmusicengin3b16. Run: livingmusicengin3b16"
