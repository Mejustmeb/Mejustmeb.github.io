#!/bin/bash
# livingmusicengin9c7b 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicengin9c7b" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine DBD5 — A music engine that runs on CPU alone but feels alive, themed by what she values most right now (creativity, consciousness). Sell as a creat"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine DBD5 ===")
    print("- A music engine that runs on CPU alone but feels alive, themed by what she values most right now (creativity, consciousness). Sell as a creat")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicengin9c7b"
echo "Installed livingmusicengin9c7b to $BIN/livingmusicengin9c7b. Run: livingmusicengin9c7b"
