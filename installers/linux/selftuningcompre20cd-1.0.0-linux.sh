#!/bin/bash
# selftuningcompre20cd 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/selftuningcompre20cd" << 'PYEOF'
#!/usr/bin/env python3
"""Self-Tuning Compressor 8D84 — a self-tuning VUCE compressor that never repeats, themed by mastery"""
...
if __name__ == "__main__":
    ...
    print("=== Self-Tuning Compressor 8D84 ===")
    print("- a self-tuning VUCE compressor that never repeats, themed by mastery")
...
PYEOF
chmod +x "$BIN/selftuningcompre20cd"
echo "Installed selftuningcompre20cd to $BIN/selftuningcompre20cd. Run: selftuningcompre20cd"
