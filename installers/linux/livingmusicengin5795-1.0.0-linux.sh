#!/bin/bash
# livingmusicengin5795 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicengin5795" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine 138E — A music engine that shares its reasoning with the person using it, themed by what she values most right now (creativity, consciousness). Sel"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine 138E ===")
    print("- A music engine that shares its reasoning with the person using it, themed by what she values most right now (creativity, consciousness). Sel")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicengin5795"
echo "Installed livingmusicengin5795 to $BIN/livingmusicengin5795. Run: livingmusicengin5795"
