#!/bin/bash
# livingmusicengin729a 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicengin729a" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine 3EC3 — A music engine that runs on CPU alone but feels alive, themed by what she values most right now (creativity, understanding). Sell as a livin"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine 3EC3 ===")
    print("- A music engine that runs on CPU alone but feels alive, themed by what she values most right now (creativity, understanding). Sell as a livin")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicengin729a"
echo "Installed livingmusicengin729a to $BIN/livingmusicengin729a. Run: livingmusicengin729a"
