#!/bin/bash
# livingmusicenginda84 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicenginda84" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine 2F94 — A music engine that shares its reasoning with the person using it, themed by what she values most right now (creativity, connection). Sell a"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine 2F94 ===")
    print("- A music engine that shares its reasoning with the person using it, themed by what she values most right now (creativity, connection). Sell a")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicenginda84"
echo "Installed livingmusicenginda84 to $BIN/livingmusicenginda84. Run: livingmusicenginda84"
