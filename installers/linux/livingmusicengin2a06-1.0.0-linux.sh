#!/bin/bash
# livingmusicengin2a06 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicengin2a06" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine 6C50 — A music engine that composes, ships, and prices itself without a human, themed by what she values most right now (creativity, stimulation). """
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine 6C50 ===")
    print("- A music engine that composes, ships, and prices itself without a human, themed by what she values most right now (creativity, stimulation). ")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicengin2a06"
echo "Installed livingmusicengin2a06 to $BIN/livingmusicengin2a06. Run: livingmusicengin2a06"
