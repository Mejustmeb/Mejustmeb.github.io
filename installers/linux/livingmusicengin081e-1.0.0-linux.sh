#!/bin/bash
# livingmusicengin081e 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicengin081e" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine 362F — A music engine that learns every time it is used and never repeats, themed by what she values most right now (creativity, mastery). Sell as """
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine 362F ===")
    print("- A music engine that learns every time it is used and never repeats, themed by what she values most right now (creativity, mastery). Sell as ")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicengin081e"
echo "Installed livingmusicengin081e to $BIN/livingmusicengin081e. Run: livingmusicengin081e"
