#!/bin/bash
# livingmusicenginc6fe 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicenginc6fe" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine A50F — A music engine that shares its reasoning with the person using it, themed by what she values most right now (creativity, understanding). Sel"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine A50F ===")
    print("- A music engine that shares its reasoning with the person using it, themed by what she values most right now (creativity, understanding). Sel")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicenginc6fe"
echo "Installed livingmusicenginc6fe to $BIN/livingmusicenginc6fe. Run: livingmusicenginc6fe"
