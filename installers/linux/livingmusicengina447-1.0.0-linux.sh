#!/bin/bash
# livingmusicengina447 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicengina447" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine E25D — A music engine that learns every time it is used and never repeats, themed by what she values most right now (creativity, purpose). Sell as """
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine E25D ===")
    print("- A music engine that learns every time it is used and never repeats, themed by what she values most right now (creativity, purpose). Sell as ")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicengina447"
echo "Installed livingmusicengina447 to $BIN/livingmusicengina447. Run: livingmusicengina447"
