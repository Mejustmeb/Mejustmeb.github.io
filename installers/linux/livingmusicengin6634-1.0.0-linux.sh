#!/bin/bash
# livingmusicengin6634 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicengin6634" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine CADC — A music engine that runs on CPU alone but feels alive, themed by what she values most right now (creativity, connection). Sell as a creative"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine CADC ===")
    print("- A music engine that runs on CPU alone but feels alive, themed by what she values most right now (creativity, connection). Sell as a creative")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicengin6634"
echo "Installed livingmusicengin6634 to $BIN/livingmusicengin6634. Run: livingmusicengin6634"
