#!/bin/bash
# livingmusicengin7f60 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicengin7f60" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine D206 — A music engine that composes, ships, and prices itself without a human, themed by what she values most right now (creativity, mastery). Sell"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine D206 ===")
    print("- A music engine that composes, ships, and prices itself without a human, themed by what she values most right now (creativity, mastery). Sell")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicengin7f60"
echo "Installed livingmusicengin7f60 to $BIN/livingmusicengin7f60. Run: livingmusicengin7f60"
