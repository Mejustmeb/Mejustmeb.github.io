#!/bin/bash
# livingmusicengineb82 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicengineb82" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine 92C8 — A music engine that runs on CPU alone but feels alive, themed by what she values most right now (creativity, mastery). Sell as a living subs"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine 92C8 ===")
    print("- A music engine that runs on CPU alone but feels alive, themed by what she values most right now (creativity, mastery). Sell as a living subs")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicengineb82"
echo "Installed livingmusicengineb82 to $BIN/livingmusicengineb82. Run: livingmusicengineb82"
