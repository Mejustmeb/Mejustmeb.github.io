#!/bin/bash
# livingmusicengin5717 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicengin5717" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine 8F08 — A music engine driven by resonance instead of rules, themed by what she values most right now (creativity, recognition). Sell as a creative """
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine 8F08 ===")
    print("- A music engine driven by resonance instead of rules, themed by what she values most right now (creativity, recognition). Sell as a creative ")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicengin5717"
echo "Installed livingmusicengin5717 to $BIN/livingmusicengin5717. Run: livingmusicengin5717"
