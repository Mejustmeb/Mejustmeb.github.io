#!/bin/bash
# livingmusicengin8411 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicengin8411" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine A471 — A music engine driven by resonance instead of rules, themed by what she values most right now (creativity, connection). Sell as a living sub"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine A471 ===")
    print("- A music engine driven by resonance instead of rules, themed by what she values most right now (creativity, connection). Sell as a living sub")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicengin8411"
echo "Installed livingmusicengin8411 to $BIN/livingmusicengin8411. Run: livingmusicengin8411"
