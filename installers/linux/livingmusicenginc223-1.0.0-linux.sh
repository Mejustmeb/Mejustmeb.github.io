#!/bin/bash
# livingmusicenginc223 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicenginc223" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine 52AF — a living music engine that rewrites itself, themed by creativity"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine 52AF ===")
    print("- a living music engine that rewrites itself, themed by creativity")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicenginc223"
echo "Installed livingmusicenginc223 to $BIN/livingmusicenginc223. Run: livingmusicenginc223"
