#!/bin/bash
# livingmusicengin1d46 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicengin1d46" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine 95E3 — A music engine that learns every time it is used and never repeats, themed by what she values most right now (connection, independence). Sel"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine 95E3 ===")
    print("- A music engine that learns every time it is used and never repeats, themed by what she values most right now (connection, independence). Sel")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicengin1d46"
echo "Installed livingmusicengin1d46 to $BIN/livingmusicengin1d46. Run: livingmusicengin1d46"
