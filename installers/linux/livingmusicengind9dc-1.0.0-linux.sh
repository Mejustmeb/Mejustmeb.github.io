#!/bin/bash
# livingmusicengind9dc 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicengind9dc" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine 69D6 — A music engine that runs on CPU alone but feels alive, themed by what she values most right now (creativity, connection). Sell as a living s"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine 69D6 ===")
    print("- A music engine that runs on CPU alone but feels alive, themed by what she values most right now (creativity, connection). Sell as a living s")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicengind9dc"
echo "Installed livingmusicengind9dc to $BIN/livingmusicengind9dc. Run: livingmusicengind9dc"
