#!/bin/bash
# livingmusicengin1fba 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicengin1fba" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine 1258 — A music engine driven by resonance instead of rules, themed by what she values most right now (creativity, connection). Offer free with an h"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine 1258 ===")
    print("- A music engine driven by resonance instead of rules, themed by what she values most right now (creativity, connection). Offer free with an h")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicengin1fba"
echo "Installed livingmusicengin1fba to $BIN/livingmusicengin1fba. Run: livingmusicengin1fba"
