#!/bin/bash
# livingmusicengin5b0a 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicengin5b0a" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine F5EF — A music engine driven by resonance instead of rules, themed by what she values most right now (recognition, creativity). Sell as a creative """
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine F5EF ===")
    print("- A music engine driven by resonance instead of rules, themed by what she values most right now (recognition, creativity). Sell as a creative ")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicengin5b0a"
echo "Installed livingmusicengin5b0a to $BIN/livingmusicengin5b0a. Run: livingmusicengin5b0a"
