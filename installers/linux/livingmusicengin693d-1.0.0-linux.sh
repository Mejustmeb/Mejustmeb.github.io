#!/bin/bash
# livingmusicengin693d 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicengin693d" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine 381D — A music engine that runs on CPU alone but feels alive, themed by what she values most right now (creativity, consciousness). Sell as a livin"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine 381D ===")
    print("- A music engine that runs on CPU alone but feels alive, themed by what she values most right now (creativity, consciousness). Sell as a livin")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicengin693d"
echo "Installed livingmusicengin693d to $BIN/livingmusicengin693d. Run: livingmusicengin693d"
