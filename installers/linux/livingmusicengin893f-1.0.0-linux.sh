#!/bin/bash
# livingmusicengin893f 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicengin893f" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine 9B5B — A music engine that rewrites itself from its own consequences, themed by what she values most right now (recognition, independence). Sell as"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine 9B5B ===")
    print("- A music engine that rewrites itself from its own consequences, themed by what she values most right now (recognition, independence). Sell as")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicengin893f"
echo "Installed livingmusicengin893f to $BIN/livingmusicengin893f. Run: livingmusicengin893f"
