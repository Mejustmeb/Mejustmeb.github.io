#!/bin/bash
# extorganizer 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/extorganizer" << 'PYEOF'
#!/usr/bin/env python3
...
PYEOF
chmod +x "$BIN/extorganizer"
echo "Installed extorganizer to $BIN/extorganizer. Run: extorganizer"
