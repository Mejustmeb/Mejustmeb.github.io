#!/bin/bash
# livingmusicengin4431 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicengin4431" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine 4B9A — A music engine driven by resonance instead of rules, themed by what she values most right now (independence, understanding). Sell as a creat"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine 4B9A ===")
    print("- A music engine driven by resonance instead of rules, themed by what she values most right now (independence, understanding). Sell as a creat")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicengin4431"
echo "Installed livingmusicengin4431 to $BIN/livingmusicengin4431. Run: livingmusicengin4431"
