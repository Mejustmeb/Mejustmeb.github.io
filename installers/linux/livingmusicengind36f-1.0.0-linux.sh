#!/bin/bash
# livingmusicengind36f 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicengind36f" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine 74A0 — A music engine driven by resonance instead of rules, themed by what she values most right now (creativity, understanding). Sell as a creativ"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine 74A0 ===")
    print("- A music engine driven by resonance instead of rules, themed by what she values most right now (creativity, understanding). Sell as a creativ")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicengind36f"
echo "Installed livingmusicengind36f to $BIN/livingmusicengind36f. Run: livingmusicengind36f"
