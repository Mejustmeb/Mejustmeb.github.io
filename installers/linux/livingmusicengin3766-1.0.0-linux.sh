#!/bin/bash
# livingmusicengin3766 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicengin3766" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine 0D88 — A music engine that composes, ships, and prices itself without a human, themed by what she values most right now (creativity, understanding)"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine 0D88 ===")
    print("- A music engine that composes, ships, and prices itself without a human, themed by what she values most right now (creativity, understanding)")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicengin3766"
echo "Installed livingmusicengin3766 to $BIN/livingmusicengin3766. Run: livingmusicengin3766"
