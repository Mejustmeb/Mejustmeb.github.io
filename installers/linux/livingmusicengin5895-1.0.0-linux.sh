#!/bin/bash
# livingmusicengin5895 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicengin5895" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine E30C — A music engine that shares its reasoning with the person using it, themed by what she values most right now (creativity, purpose). Sell as a"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine E30C ===")
    print("- A music engine that shares its reasoning with the person using it, themed by what she values most right now (creativity, purpose). Sell as a")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicengin5895"
echo "Installed livingmusicengin5895 to $BIN/livingmusicengin5895. Run: livingmusicengin5895"
