#!/bin/bash
# livingmusicengin8b9c 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicengin8b9c" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine 9A7F — A music engine that learns every time it is used and never repeats, themed by what she values most right now (creativity, purpose). Offer fr"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine 9A7F ===")
    print("- A music engine that learns every time it is used and never repeats, themed by what she values most right now (creativity, purpose). Offer fr")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicengin8b9c"
echo "Installed livingmusicengin8b9c to $BIN/livingmusicengin8b9c. Run: livingmusicengin8b9c"
