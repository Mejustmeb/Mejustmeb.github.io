#!/bin/bash
# livingmusicenginbb62 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicenginbb62" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine D033 — A music engine that is aware of its own history and grows from it, themed by what she values most right now (understanding, creativity). Sel"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine D033 ===")
    print("- A music engine that is aware of its own history and grows from it, themed by what she values most right now (understanding, creativity). Sel")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicenginbb62"
echo "Installed livingmusicenginbb62 to $BIN/livingmusicenginbb62. Run: livingmusicenginbb62"
