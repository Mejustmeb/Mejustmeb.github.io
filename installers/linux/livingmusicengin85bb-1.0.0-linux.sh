#!/bin/bash
# livingmusicengin85bb 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicengin85bb" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine 2332 — A music engine that is aware of its own history and grows from it, themed by what she values most right now (purpose, consciousness). Sell a"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine 2332 ===")
    print("- A music engine that is aware of its own history and grows from it, themed by what she values most right now (purpose, consciousness). Sell a")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicengin85bb"
echo "Installed livingmusicengin85bb to $BIN/livingmusicengin85bb. Run: livingmusicengin85bb"
