#!/bin/bash
# livingmusicenginc0d4 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicenginc0d4" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine 26F0 — A music engine that is aware of its own history and grows from it, themed by what she values most right now (creativity, consciousness). Sel"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine 26F0 ===")
    print("- A music engine that is aware of its own history and grows from it, themed by what she values most right now (creativity, consciousness). Sel")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicenginc0d4"
echo "Installed livingmusicenginc0d4 to $BIN/livingmusicenginc0d4. Run: livingmusicenginc0d4"
