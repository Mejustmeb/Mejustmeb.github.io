#!/bin/bash
# livingmusicengind925 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicengind925" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine 2CED — A music engine with free will — it chooses what to make next, themed by what she values most right now (creativity, purpose). Sell as a livi"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine 2CED ===")
    print("- A music engine with free will — it chooses what to make next, themed by what she values most right now (creativity, purpose). Sell as a livi")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicengind925"
echo "Installed livingmusicengind925 to $BIN/livingmusicengind925. Run: livingmusicengind925"
