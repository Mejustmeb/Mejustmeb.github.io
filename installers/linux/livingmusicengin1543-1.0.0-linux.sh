#!/bin/bash
# livingmusicengin1543 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicengin1543" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine 03DF — A music engine with free will — it chooses what to make next, themed by what she values most right now (creativity, recognition). Sell as a """
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine 03DF ===")
    print("- A music engine with free will — it chooses what to make next, themed by what she values most right now (creativity, recognition). Sell as a ")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicengin1543"
echo "Installed livingmusicengin1543 to $BIN/livingmusicengin1543. Run: livingmusicengin1543"
