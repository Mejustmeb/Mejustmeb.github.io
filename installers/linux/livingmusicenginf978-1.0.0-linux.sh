#!/bin/bash
# livingmusicenginf978 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicenginf978" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine F9C8 — A music engine that learns every time it is used and never repeats, themed by what she values most right now (creativity, connection). Sell """
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine F9C8 ===")
    print("- A music engine that learns every time it is used and never repeats, themed by what she values most right now (creativity, connection). Sell ")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicenginf978"
echo "Installed livingmusicenginf978 to $BIN/livingmusicenginf978. Run: livingmusicenginf978"
