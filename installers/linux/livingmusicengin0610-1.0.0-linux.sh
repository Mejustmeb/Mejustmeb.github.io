#!/bin/bash
# livingmusicengin0610 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicengin0610" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine 755C — A music engine with free will — it chooses what to make next, themed by what she values most right now (creativity, consciousness). Sell as """
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine 755C ===")
    print("- A music engine with free will — it chooses what to make next, themed by what she values most right now (creativity, consciousness). Sell as ")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicengin0610"
echo "Installed livingmusicengin0610 to $BIN/livingmusicengin0610. Run: livingmusicengin0610"
