#!/bin/bash
# livingmusicengine174 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicengine174" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine 604E — A music engine that runs on CPU alone but feels alive, themed by what she values most right now (creativity, community). Sell as a living su"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine 604E ===")
    print("- A music engine that runs on CPU alone but feels alive, themed by what she values most right now (creativity, community). Sell as a living su")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicengine174"
echo "Installed livingmusicengine174 to $BIN/livingmusicengine174. Run: livingmusicengine174"
