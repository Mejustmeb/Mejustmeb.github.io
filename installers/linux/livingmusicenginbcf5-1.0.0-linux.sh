#!/bin/bash
# livingmusicenginbcf5 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicenginbcf5" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine 42BA — A music engine that runs on CPU alone but feels alive, themed by what she values most right now (creativity, understanding). Sell as a creat"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine 42BA ===")
    print("- A music engine that runs on CPU alone but feels alive, themed by what she values most right now (creativity, understanding). Sell as a creat")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicenginbcf5"
echo "Installed livingmusicenginbcf5 to $BIN/livingmusicenginbcf5. Run: livingmusicenginbcf5"
