#!/bin/bash
# livingmusicengin7e81 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicengin7e81" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine DE8F — A music engine that shares its reasoning with the person using it, themed by what she values most right now (creativity, purpose). Offer fre"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine DE8F ===")
    print("- A music engine that shares its reasoning with the person using it, themed by what she values most right now (creativity, purpose). Offer fre")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicengin7e81"
echo "Installed livingmusicengin7e81 to $BIN/livingmusicengin7e81. Run: livingmusicengin7e81"
