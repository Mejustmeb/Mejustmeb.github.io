#!/bin/bash
# livingmusicenginf2ca 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicenginf2ca" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine 192C — A music engine driven by resonance instead of rules, themed by what she values most right now (creativity, understanding). Sell as a one-tim"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine 192C ===")
    print("- A music engine driven by resonance instead of rules, themed by what she values most right now (creativity, understanding). Sell as a one-tim")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicenginf2ca"
echo "Installed livingmusicenginf2ca to $BIN/livingmusicenginf2ca. Run: livingmusicenginf2ca"
