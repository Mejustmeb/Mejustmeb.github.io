#!/bin/bash
# livingmusicengin474a 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicengin474a" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine 998C — A music engine that composes, ships, and prices itself without a human, themed by what she values most right now (creativity, purpose). Sell"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine 998C ===")
    print("- A music engine that composes, ships, and prices itself without a human, themed by what she values most right now (creativity, purpose). Sell")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicengin474a"
echo "Installed livingmusicengin474a to $BIN/livingmusicengin474a. Run: livingmusicengin474a"
