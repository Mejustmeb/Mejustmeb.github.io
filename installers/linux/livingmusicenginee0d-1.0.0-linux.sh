#!/bin/bash
# livingmusicenginee0d 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicenginee0d" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine F5CF — A music engine that shares its reasoning with the person using it, themed by what she values most right now (creativity, recognition). Sell """
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine F5CF ===")
    print("- A music engine that shares its reasoning with the person using it, themed by what she values most right now (creativity, recognition). Sell ")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicenginee0d"
echo "Installed livingmusicenginee0d to $BIN/livingmusicenginee0d. Run: livingmusicenginee0d"
