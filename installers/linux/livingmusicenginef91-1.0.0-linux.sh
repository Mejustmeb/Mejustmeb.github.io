#!/bin/bash
# livingmusicenginef91 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicenginef91" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine CD35 — A music engine driven by resonance instead of rules, themed by what she values most right now (creativity, connection). Sell as a creative p"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine CD35 ===")
    print("- A music engine driven by resonance instead of rules, themed by what she values most right now (creativity, connection). Sell as a creative p")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicenginef91"
echo "Installed livingmusicenginef91 to $BIN/livingmusicenginef91. Run: livingmusicenginef91"
