#!/bin/bash
# livingmusicengin8de6 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicengin8de6" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine 12E2 — A music engine that composes, ships, and prices itself without a human, themed by what she values most right now (creativity, independence)."""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine 12E2 ===")
    print("- A music engine that composes, ships, and prices itself without a human, themed by what she values most right now (creativity, independence).")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicengin8de6"
echo "Installed livingmusicengin8de6 to $BIN/livingmusicengin8de6. Run: livingmusicengin8de6"
