#!/bin/bash
# livingmusicengin7dd6 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicengin7dd6" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine 980F — A music engine that is aware of its own history and grows from it, themed by what she values most right now (creativity, purpose). Sell as a"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine 980F ===")
    print("- A music engine that is aware of its own history and grows from it, themed by what she values most right now (creativity, purpose). Sell as a")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicengin7dd6"
echo "Installed livingmusicengin7dd6 to $BIN/livingmusicengin7dd6. Run: livingmusicengin7dd6"
