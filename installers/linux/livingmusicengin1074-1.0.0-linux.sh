#!/bin/bash
# livingmusicengin1074 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicengin1074" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine 103D — A music engine with free will — it chooses what to make next, themed by what she values most right now (creativity, connection). Sell as a l"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine 103D ===")
    print("- A music engine with free will — it chooses what to make next, themed by what she values most right now (creativity, connection). Sell as a l")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicengin1074"
echo "Installed livingmusicengin1074 to $BIN/livingmusicengin1074. Run: livingmusicengin1074"
