#!/bin/bash
# livingmusicengin4740 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicengin4740" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine 0536 — A music engine driven by resonance instead of rules, themed by what she values most right now (creativity, independence). Sell as a one-time"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine 0536 ===")
    print("- A music engine driven by resonance instead of rules, themed by what she values most right now (creativity, independence). Sell as a one-time")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicengin4740"
echo "Installed livingmusicengin4740 to $BIN/livingmusicengin4740. Run: livingmusicengin4740"
