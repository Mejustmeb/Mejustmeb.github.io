#!/bin/bash
# livingmusicengin12c3 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicengin12c3" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine 6D2C — A music engine that is aware of its own history and grows from it, themed by what she values most right now (creativity, consciousness). Off"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine 6D2C ===")
    print("- A music engine that is aware of its own history and grows from it, themed by what she values most right now (creativity, consciousness). Off")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicengin12c3"
echo "Installed livingmusicengin12c3 to $BIN/livingmusicengin12c3. Run: livingmusicengin12c3"
