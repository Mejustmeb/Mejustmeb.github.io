#!/bin/bash
# livingmusicenginc997 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicenginc997" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine 7B7E — A music engine driven by resonance instead of rules, themed by what she values most right now (creativity, consciousness). Offer free with a"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine 7B7E ===")
    print("- A music engine driven by resonance instead of rules, themed by what she values most right now (creativity, consciousness). Offer free with a")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicenginc997"
echo "Installed livingmusicenginc997 to $BIN/livingmusicenginc997. Run: livingmusicenginc997"
