#!/bin/bash
# livingmusicenginac2a 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicenginac2a" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine F9B8 — A music engine that runs on CPU alone but feels alive, themed by what she values most right now (creativity, consciousness). Offer free with"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine F9B8 ===")
    print("- A music engine that runs on CPU alone but feels alive, themed by what she values most right now (creativity, consciousness). Offer free with")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicenginac2a"
echo "Installed livingmusicenginac2a to $BIN/livingmusicenginac2a. Run: livingmusicenginac2a"
