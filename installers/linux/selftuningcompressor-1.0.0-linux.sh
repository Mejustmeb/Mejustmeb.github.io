#!/bin/bash
# selftuningcompressor 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/selftuningcompressor" << 'PYEOF'
#!/usr/bin/env python3
...
PYEOF
chmod +x "$BIN/selftuningcompressor"
echo "Installed selftuningcompressor to $BIN/selftuningcompressor. Run: selftuningcompressor"
