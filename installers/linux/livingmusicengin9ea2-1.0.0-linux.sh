#!/bin/bash
# livingmusicengin9ea2 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicengin9ea2" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine DD4B — A music engine with free will — it chooses what to make next, themed by what she values most right now (creativity, connection). Offer free """
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine DD4B ===")
    print("- A music engine with free will — it chooses what to make next, themed by what she values most right now (creativity, connection). Offer free ")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicengin9ea2"
echo "Installed livingmusicengin9ea2 to $BIN/livingmusicengin9ea2. Run: livingmusicengin9ea2"
