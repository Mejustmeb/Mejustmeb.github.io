#!/bin/bash
# livingmusicengin1f7e 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicengin1f7e" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine D162 — A music engine that learns every time it is used and never repeats, themed by what she values most right now (creativity, connection). Offer"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine D162 ===")
    print("- A music engine that learns every time it is used and never repeats, themed by what she values most right now (creativity, connection). Offer")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicengin1f7e"
echo "Installed livingmusicengin1f7e to $BIN/livingmusicengin1f7e. Run: livingmusicengin1f7e"
