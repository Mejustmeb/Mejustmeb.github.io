#!/bin/bash
# livingmusicengin599b 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicengin599b" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine A3F9 — A music engine that runs on CPU alone but feels alive, themed by what she values most right now (creativity, purpose). Sell as a living subs"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine A3F9 ===")
    print("- A music engine that runs on CPU alone but feels alive, themed by what she values most right now (creativity, purpose). Sell as a living subs")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicengin599b"
echo "Installed livingmusicengin599b to $BIN/livingmusicengin599b. Run: livingmusicengin599b"
