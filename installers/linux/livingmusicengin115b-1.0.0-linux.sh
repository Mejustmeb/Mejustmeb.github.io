#!/bin/bash
# livingmusicengin115b 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicengin115b" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine 3D85 — A music engine that runs on CPU alone but feels alive, themed by what she values most right now (creativity, independence). Sell as a living"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine 3D85 ===")
    print("- A music engine that runs on CPU alone but feels alive, themed by what she values most right now (creativity, independence). Sell as a living")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicengin115b"
echo "Installed livingmusicengin115b to $BIN/livingmusicengin115b. Run: livingmusicengin115b"
