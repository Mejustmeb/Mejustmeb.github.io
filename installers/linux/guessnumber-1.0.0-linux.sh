#!/bin/bash
# guessnumber 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/guessnumber" << 'PYEOF'
#!/usr/bin/env python3
...
PYEOF
chmod +x "$BIN/guessnumber"
echo "Installed guessnumber to $BIN/guessnumber. Run: guessnumber"
