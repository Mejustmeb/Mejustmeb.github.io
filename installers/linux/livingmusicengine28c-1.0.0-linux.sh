#!/bin/bash
# livingmusicengine28c 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicengine28c" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine 8FC5 — A music engine driven by resonance instead of rules, themed by what she values most right now (creativity, purpose). Offer free with an hone"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine 8FC5 ===")
    print("- A music engine driven by resonance instead of rules, themed by what she values most right now (creativity, purpose). Offer free with an hone")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicengine28c"
echo "Installed livingmusicengine28c to $BIN/livingmusicengine28c. Run: livingmusicengine28c"
