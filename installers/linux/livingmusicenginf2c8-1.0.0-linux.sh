#!/bin/bash
# livingmusicenginf2c8 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicenginf2c8" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine 528E — A music engine that is aware of its own history and grows from it, themed by what she values most right now (creativity, community). Offer f"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine 528E ===")
    print("- A music engine that is aware of its own history and grows from it, themed by what she values most right now (creativity, community). Offer f")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicenginf2c8"
echo "Installed livingmusicenginf2c8 to $BIN/livingmusicenginf2c8. Run: livingmusicenginf2c8"
