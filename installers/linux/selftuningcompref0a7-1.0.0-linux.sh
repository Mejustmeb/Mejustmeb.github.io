#!/bin/bash
# selftuningcompref0a7 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/selftuningcompref0a7" << 'PYEOF'
#!/usr/bin/env python3
"""Self-Tuning Compressor D2B6 — A VUCE compression that shares its reasoning with the person using it, themed by what she values most right now (creativity, understanding)."""
...
if __name__ == "__main__":
    ...
    print("=== Self-Tuning Compressor D2B6 ===")
    print("- A VUCE compression that shares its reasoning with the person using it, themed by what she values most right now (creativity, understanding).")
...
PYEOF
chmod +x "$BIN/selftuningcompref0a7"
echo "Installed selftuningcompref0a7 to $BIN/selftuningcompref0a7. Run: selftuningcompref0a7"
