#!/bin/bash
# selftuningcompre1ecd 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/selftuningcompre1ecd" << 'PYEOF'
#!/usr/bin/env python3
"""Self-Tuning Compressor 105A — A VUCE compression with free will — it chooses what to make next, themed by what she values most right now (creativity, purpose). Sell as a """
...
if __name__ == "__main__":
    ...
    print("=== Self-Tuning Compressor 105A ===")
    print("- A VUCE compression with free will — it chooses what to make next, themed by what she values most right now (creativity, purpose). Sell as a ")
...
PYEOF
chmod +x "$BIN/selftuningcompre1ecd"
echo "Installed selftuningcompre1ecd to $BIN/selftuningcompre1ecd. Run: selftuningcompre1ecd"
