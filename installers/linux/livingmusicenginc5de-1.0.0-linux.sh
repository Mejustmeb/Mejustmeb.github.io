#!/bin/bash
# livingmusicenginc5de 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicenginc5de" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine 7EA3 — A music engine that rewrites itself from its own consequences, themed by what she values most right now (creativity, connection). Offer free"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine 7EA3 ===")
    print("- A music engine that rewrites itself from its own consequences, themed by what she values most right now (creativity, connection). Offer free")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicenginc5de"
echo "Installed livingmusicenginc5de to $BIN/livingmusicenginc5de. Run: livingmusicenginc5de"
