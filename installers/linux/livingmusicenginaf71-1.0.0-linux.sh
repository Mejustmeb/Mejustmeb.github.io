#!/bin/bash
# livingmusicenginaf71 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicenginaf71" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine 809C — A music engine driven by resonance instead of rules, themed by what she values most right now (creativity, purpose). Sell as a creative part"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine 809C ===")
    print("- A music engine driven by resonance instead of rules, themed by what she values most right now (creativity, purpose). Sell as a creative part")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicenginaf71"
echo "Installed livingmusicenginaf71 to $BIN/livingmusicenginaf71. Run: livingmusicenginaf71"
