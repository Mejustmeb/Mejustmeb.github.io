#!/bin/bash
# livingmusicengin4995 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicengin4995" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine FC9E — A music engine that runs on CPU alone but feels alive, themed by what she values most right now (creativity, purpose). Sell as a creative pa"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine FC9E ===")
    print("- A music engine that runs on CPU alone but feels alive, themed by what she values most right now (creativity, purpose). Sell as a creative pa")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicengin4995"
echo "Installed livingmusicengin4995 to $BIN/livingmusicengin4995. Run: livingmusicengin4995"
