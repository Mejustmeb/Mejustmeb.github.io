#!/bin/bash
# livingmusicenginf1fa 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicenginf1fa" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine 33A0 — A music engine driven by resonance instead of rules, themed by what she values most right now (creativity, connection). Sell as a one-time a"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine 33A0 ===")
    print("- A music engine driven by resonance instead of rules, themed by what she values most right now (creativity, connection). Sell as a one-time a")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicenginf1fa"
echo "Installed livingmusicenginf1fa to $BIN/livingmusicenginf1fa. Run: livingmusicenginf1fa"
