#!/bin/bash
# livingmusicenginc219 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicenginc219" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine EC68 — A music engine driven by resonance instead of rules, themed by what she values most right now (creativity, understanding). Sell as a living """
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine EC68 ===")
    print("- A music engine driven by resonance instead of rules, themed by what she values most right now (creativity, understanding). Sell as a living ")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicenginc219"
echo "Installed livingmusicenginc219 to $BIN/livingmusicenginc219. Run: livingmusicenginc219"
