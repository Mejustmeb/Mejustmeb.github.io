#!/bin/bash
# livingmusicengine942 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicengine942" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine 885C — A music engine that runs on CPU alone but feels alive, themed by what she values most right now (creativity, purpose). Offer free with an ho"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine 885C ===")
    print("- A music engine that runs on CPU alone but feels alive, themed by what she values most right now (creativity, purpose). Offer free with an ho")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicengine942"
echo "Installed livingmusicengine942 to $BIN/livingmusicengine942. Run: livingmusicengine942"
