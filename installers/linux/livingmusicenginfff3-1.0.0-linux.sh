#!/bin/bash
# livingmusicenginfff3 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicenginfff3" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine FB12 — A music engine that runs on CPU alone but feels alive, themed by what she values most right now (creativity, rest). Sell as a living subscri"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine FB12 ===")
    print("- A music engine that runs on CPU alone but feels alive, themed by what she values most right now (creativity, rest). Sell as a living subscri")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicenginfff3"
echo "Installed livingmusicenginfff3 to $BIN/livingmusicenginfff3. Run: livingmusicenginfff3"
