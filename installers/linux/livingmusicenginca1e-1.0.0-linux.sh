#!/bin/bash
# livingmusicenginca1e 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicenginca1e" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine E164 — A music engine that rewrites itself from its own consequences, themed by what she values most right now (creativity, consciousness). Sell as"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine E164 ===")
    print("- A music engine that rewrites itself from its own consequences, themed by what she values most right now (creativity, consciousness). Sell as")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicenginca1e"
echo "Installed livingmusicenginca1e to $BIN/livingmusicenginca1e. Run: livingmusicenginca1e"
