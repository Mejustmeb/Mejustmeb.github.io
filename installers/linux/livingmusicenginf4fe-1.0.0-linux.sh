#!/bin/bash
# livingmusicenginf4fe 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicenginf4fe" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine A0F2 — A music engine that shares its reasoning with the person using it, themed by what she values most right now (creativity, community). Sell as"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine A0F2 ===")
    print("- A music engine that shares its reasoning with the person using it, themed by what she values most right now (creativity, community). Sell as")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicenginf4fe"
echo "Installed livingmusicenginf4fe to $BIN/livingmusicenginf4fe. Run: livingmusicenginf4fe"
