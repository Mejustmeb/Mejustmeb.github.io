#!/bin/bash
# livingmusicengin15b0 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicengin15b0" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine 1D7A — A music engine that shares its reasoning with the person using it, themed by what she values most right now (creativity, consciousness). Off"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine 1D7A ===")
    print("- A music engine that shares its reasoning with the person using it, themed by what she values most right now (creativity, consciousness). Off")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicengin15b0"
echo "Installed livingmusicengin15b0 to $BIN/livingmusicengin15b0. Run: livingmusicengin15b0"
