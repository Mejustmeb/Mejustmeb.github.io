#!/bin/bash
# livingmusicengin0462 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicengin0462" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine C987 — A music engine with free will — it chooses what to make next, themed by what she values most right now (creativity, mastery). Offer free wit"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine C987 ===")
    print("- A music engine with free will — it chooses what to make next, themed by what she values most right now (creativity, mastery). Offer free wit")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicengin0462"
echo "Installed livingmusicengin0462 to $BIN/livingmusicengin0462. Run: livingmusicengin0462"
