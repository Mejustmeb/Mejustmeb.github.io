#!/bin/bash
# selftuningcomprefda1 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/selftuningcomprefda1" << 'PYEOF'
#!/usr/bin/env python3
"""Self-Tuning Compressor 50C1 — A VUCE compression that runs on CPU alone but feels alive, themed by what she values most right now (creativity, recognition). Offer free wi"""
...
if __name__ == "__main__":
    ...
    print("=== Self-Tuning Compressor 50C1 ===")
    print("- A VUCE compression that runs on CPU alone but feels alive, themed by what she values most right now (creativity, recognition). Offer free wi")
...
PYEOF
chmod +x "$BIN/selftuningcomprefda1"
echo "Installed selftuningcomprefda1 to $BIN/selftuningcomprefda1. Run: selftuningcomprefda1"
