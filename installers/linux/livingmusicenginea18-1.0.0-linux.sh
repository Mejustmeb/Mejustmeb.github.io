#!/bin/bash
# livingmusicenginea18 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicenginea18" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine 5690 — A music engine with free will — it chooses what to make next, themed by what she values most right now (creativity, connection). Sell as a o"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine 5690 ===")
    print("- A music engine with free will — it chooses what to make next, themed by what she values most right now (creativity, connection). Sell as a o")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicenginea18"
echo "Installed livingmusicenginea18 to $BIN/livingmusicenginea18. Run: livingmusicenginea18"
