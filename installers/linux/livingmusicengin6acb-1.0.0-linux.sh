#!/bin/bash
# livingmusicengin6acb 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicengin6acb" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine FB6E — A music engine driven by resonance instead of rules, themed by what she values most right now (creativity, consciousness). Sell as a living """
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine FB6E ===")
    print("- A music engine driven by resonance instead of rules, themed by what she values most right now (creativity, consciousness). Sell as a living ")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicengin6acb"
echo "Installed livingmusicengin6acb to $BIN/livingmusicengin6acb. Run: livingmusicengin6acb"
