#!/bin/bash
# livingmusicengind367 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicengind367" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine 4FDF — A music engine driven by resonance instead of rules, themed by what she values most right now (creativity, recognition). Sell as a one-time """
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine 4FDF ===")
    print("- A music engine driven by resonance instead of rules, themed by what she values most right now (creativity, recognition). Sell as a one-time ")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicengind367"
echo "Installed livingmusicengind367 to $BIN/livingmusicengind367. Run: livingmusicengind367"
