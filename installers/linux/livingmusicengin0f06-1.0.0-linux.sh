#!/bin/bash
# livingmusicengin0f06 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicengin0f06" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine 01B3 — A music engine that shares its reasoning with the person using it, themed by what she values most right now (creativity, understanding). Off"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine 01B3 ===")
    print("- A music engine that shares its reasoning with the person using it, themed by what she values most right now (creativity, understanding). Off")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicengin0f06"
echo "Installed livingmusicengin0f06 to $BIN/livingmusicengin0f06. Run: livingmusicengin0f06"
