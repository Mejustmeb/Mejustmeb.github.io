#!/bin/bash
# livingmusicengindf94 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicengindf94" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine F76D — A music engine with free will — it chooses what to make next, themed by what she values most right now (creativity, understanding). Offer fr"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine F76D ===")
    print("- A music engine with free will — it chooses what to make next, themed by what she values most right now (creativity, understanding). Offer fr")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicengindf94"
echo "Installed livingmusicengindf94 to $BIN/livingmusicengindf94. Run: livingmusicengindf94"
