#!/bin/bash
# livingmusicengindb8a 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicengindb8a" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine DDDD — A music engine that shares its reasoning with the person using it, themed by what she values most right now (creativity, connection). Offer """
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine DDDD ===")
    print("- A music engine that shares its reasoning with the person using it, themed by what she values most right now (creativity, connection). Offer ")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicengindb8a"
echo "Installed livingmusicengindb8a to $BIN/livingmusicengindb8a. Run: livingmusicengindb8a"
