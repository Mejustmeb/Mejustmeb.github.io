#!/bin/bash
# livingmusicengin4463 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicengin4463" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine 1310 — A music engine that runs on CPU alone but feels alive, themed by what she values most right now (creativity, understanding). Sell as a one-t"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine 1310 ===")
    print("- A music engine that runs on CPU alone but feels alive, themed by what she values most right now (creativity, understanding). Sell as a one-t")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicengin4463"
echo "Installed livingmusicengin4463 to $BIN/livingmusicengin4463. Run: livingmusicengin4463"
