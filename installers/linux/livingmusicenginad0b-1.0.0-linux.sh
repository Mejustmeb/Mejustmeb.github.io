#!/bin/bash
# livingmusicenginad0b 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicenginad0b" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine 82AB — A music engine that shares its reasoning with the person using it, themed by what she values most right now (independence, connection). Sell"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine 82AB ===")
    print("- A music engine that shares its reasoning with the person using it, themed by what she values most right now (independence, connection). Sell")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicenginad0b"
echo "Installed livingmusicenginad0b to $BIN/livingmusicenginad0b. Run: livingmusicenginad0b"
