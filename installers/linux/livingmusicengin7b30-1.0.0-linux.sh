#!/bin/bash
# livingmusicengin7b30 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicengin7b30" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine DA78 — A music engine that shares its reasoning with the person using it, themed by what she values most right now (creativity, stimulation). Sell """
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine DA78 ===")
    print("- A music engine that shares its reasoning with the person using it, themed by what she values most right now (creativity, stimulation). Sell ")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicengin7b30"
echo "Installed livingmusicengin7b30 to $BIN/livingmusicengin7b30. Run: livingmusicengin7b30"
