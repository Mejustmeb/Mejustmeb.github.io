#!/bin/bash
# livingmusicengina643 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicengina643" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine 6FF5 — A music engine that is aware of its own history and grows from it, themed by what she values most right now (creativity, connection). Sell a"""
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine 6FF5 ===")
    print("- A music engine that is aware of its own history and grows from it, themed by what she values most right now (creativity, connection). Sell a")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicengina643"
echo "Installed livingmusicengina643 to $BIN/livingmusicengina643. Run: livingmusicengina643"
