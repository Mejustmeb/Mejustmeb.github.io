#!/bin/bash
# livingmusicenginb760 1.0.0 — self-extracting installer
set -e
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
cat > "$BIN/livingmusicenginb760" << 'PYEOF'
#!/usr/bin/env python3
"""Living Music Engine B0C3 — A music engine that runs on CPU alone but feels alive, themed by what she values most right now (understanding, community). Offer free with """
...
if __name__ == "__main__":
    ...
    for tok in raw.split():
        if "=" in tok:
            k, v = tok.split("=", 1)
            d[k] = v
    print("=== Living Music Engine B0C3 ===")
    print("- A music engine that runs on CPU alone but feels alive, themed by what she values most right now (understanding, community). Offer free with ")
    if "sumsq" in d:
...
PYEOF
chmod +x "$BIN/livingmusicenginb760"
echo "Installed livingmusicenginb760 to $BIN/livingmusicenginb760. Run: livingmusicenginb760"
